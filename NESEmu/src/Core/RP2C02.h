...
namespace NESEmu {
	...
	private:
		...
		uint16_t mCurrentVRAMAddress = 0x0000; // v Current VRAM address(15 bits)
		uint16_t mTemporaryVRAMAddress = 0x0000; // t Temporary VRAM address(15 bits); can also be thought of as the address of the top left onscreen tile.
		uint8_t mFineXScroll = 0x00; // x Fine X scroll(3 bits)
		bool mWriteToggle = false; // w First or second write toggle(1 bit)
